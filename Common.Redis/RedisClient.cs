...
using RedisErrorEventArgs = Common.Redis.Args.RedisErrorEventArgs;
...
namespace Common.Redis
{
    public class RedisClient : IRedisClient
    {
        ...
        public RedisClient(RedisConfig config) : this(
            $"{config.Host}:{config.Port},abortConnect=false,defaultDatabase={config.Database},ssl=false,ConnectTimeout={config.ConnectTimeout},allowAdmin=true,connectRetry={config.ConnectRetry}")
        {
        }
        ...
        public EventHandler<RedisErrorEventArgs> RedisErrorEvent { get; set; }
        ...
        private void Redis_ErrorMessage(object sender, StackExchange.Redis.RedisErrorEventArgs e)
        {
            RedisErrorEvent?.Invoke(sender, new RedisErrorEventArgs(e.EndPoint.ToString(), e.Message));
        }
    }
}